...
import com.modcrafting.diablodrops.SockettingSettings;
import com.modcrafting.diablodrops.items.GemType;
import com.modcrafting.diablodrops.items.ItemUtil;
import com.modcrafting.diablodrops.items.SocketEffect;
import com.modcrafting.diablodrops.items.SocketGem;
import com.modcrafting.diablodrops.items.SocketGemUtil;
import com.modcrafting.diablodrops.tier.MythicDropsTier;
import com.modcrafting.diablodrops.tier.TierUtil;
...
public final class SockettingListener implements Listener {
    ...
    private DiabloDrops mythicDrops;

    public SockettingListener(DiabloDrops mythicDrops) {
        this.mythicDrops = mythicDrops;
    }

    public DiabloDrops getMythicDrops() {
        return mythicDrops;
    }
	...
	private void socketItem(PlayerInteractEvent event, Player player, ItemStack itemInHand,
        ...
        if (ItemUtil.isArmor(itemType) || ItemUtil.isTool(itemType)) {
            ...
            MythicDropsTier tier = TierUtil.getTierFromItemStack(itemInHand);

            ChatColor cc = tier != null ? tier.getDisplayColor() : ChatColor.GOLD;

            lore.set(index, cc + socketGem.getName());
            
            List<String> colorCoded = mythicDrops.getSockettingSettings().getSockettedItemLore();
            
            int i = 0;
            for (String c : colorCoded) {
            	i++;
            	colorCoded.set(i, ChatColor.translateAlternateColorCodes('&', c));
            }
