...
public class EffectAPI {
	...
	public static HashMap<String, Integer> getEffectsFromItem(ItemStack a) {
		...
		if (a.getItemMeta() != null && a.getItemMeta().getLore() != null
				&& (!a.getItemMeta().getLore().get(0).equals("Socket")) && a.getItemMeta().getLore().size() > 1
	...
	public static Effect getRandomEffect(String Type) {
		...
		switch (Type) {
		case TYPE_ARMOR:
			return new Effect(ArmorEffects.get(Plugin.Random.nextInt(ArmorEffects.size())), Type,
					Plugin.Random.nextInt(3) + 1, "0:15");
		case TYPE_WEAPON_PASSIVE:
			return new Effect(PassiveWeaponEffects.get(Plugin.Random.nextInt(PassiveWeaponEffects.size())), Type,
					Plugin.Random.nextInt(3) + 1, "0:15");
		case TYPE_WEAPON:
			return new Effect(HitWeaponEffects.get(Plugin.Random.nextInt(HitWeaponEffects.size())), Type,
					Plugin.Random.nextInt(3) + 1, "0:15");
		case TYPE_BOW:
			return new Effect(BowEffects.get(Plugin.Random.nextInt(BowEffects.size())), Type,
					Plugin.Random.nextInt(3) + 1, "0:15");
		case TYPE_TOOL:
			return new Effect(ToolEffects.get(Plugin.Random.nextInt(ToolEffects.size())), Type,
					Plugin.Random.nextInt(3) + 1, "0:15");
		}
