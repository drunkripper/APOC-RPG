...
public class ItemAPI {
	...
	private static Material[] Materials = { Material.BOW, Material.WOOD_AXE, Material.WOOD_HOE, Material.WOOD_PICKAXE,
	 ...
	 * Create Armor
	 * <p>
	 * Creates an armor item based on a passed in parameter, with random
	 * enchants and type.
	 * 
	 * @param type
	 *            0 = Helmet, 1 = Chestplate, 2 = Leggings, 3 = Boots, 4 = Held
	 *            weapons
	 ...
	 * Diablofy wrapper:
	 * <p>
	 * this will diablofy the item that is passed to it in the classic way. This
	 * is a wrapper to preserve method signature.
	 */
	 ...
	 * @param Item
	 * @param type
	 *            an Int that determines how to diablofy- 0=ignored (Normal)
	 *            1=armor (Useful) 2=weapon (Useful) 3=tool (Useful) 4=bow
	 *            (useful)
	 * @return Funky Item
	 */
	public static ItemStack diablofy(ItemStack item, int type, int tier) {
		...
		if (tierNaming) {
			...
			// suffix
			meta.setDisplayName(nameColor + (tier == Plugin.TIER_SET ? Plugin.Settings.getRandomSet() + " " : "")
					+ Plugin.Settings.getRandomPrefix() + " " + Plugin.Settings.getRandomSuffix());
		}
		...
		if (block.getType() == Material.CHEST) {
			...
			if (maxItems > 0) {
				...
				for (int i = 0; i < nbrItems; i++) {
					// if Plugin.CHEST_FILL_RPG != TRUE or chestItems is empty
					// create common items
